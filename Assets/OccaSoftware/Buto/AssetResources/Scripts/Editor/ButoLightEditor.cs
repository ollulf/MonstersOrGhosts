...
namespace OccaSoftware.Buto.Editor
{
    ...
    public class ButoLightEditor : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            /*
            if(ButoLight.Lights.Count > 8)
            {
                GUIStyle style = EditorStyles.wordWrappedLabel;
                style.fontStyle = FontStyle.Bold;
                style.normal.textColor = Color.yellow;
                EditorGUILayout.Space();
                EditorGUILayout.LabelField("WARNING \nThere are  enabled Buto Lights in scene. Some lights will not be rendered with Buto. Remove or disable some lights to avoid undefined behavior.", style);
                EditorGUILayout.Space();
            }
            */ 
