Shader "NatureManufacture Shaders/Ice Lake/Ice Shader Specular Lake Translucency"
{
	Properties
	{
		[Header(Translucency)]
		...
		_IceNoiseTiling("Ice Noise Tiling", Range( 0 , 100)) = 100
		...
		[Toggle(_SHADOWSPECULAROCCLUSION_ON)] _ShadowSpecularOcclusion("Shadow Specular Occlusion", Float) = 0
		_ShadowSpecularPower("Shadow Specular Power", Range( 0 , 1)) = 0
		_ShadowSpecularOcclusionPower("Shadow Specular Occlusion Power", Range( 0 , 10)) = 2
		_ShadowPower("Shadow Power", Range( 0 , 5)) = 2
		_ShadowRefractionPower("Shadow Refraction Power", Range( 0 , 1)) = 0.155
	...
	SubShader
	{
		...
		#pragma shader_feature _SHADOWSPECULAROCCLUSION_ON
		...
		uniform float _ShadowRefractionPower;
		uniform float _SpecularPower;
		uniform float _ShadowSpecularPower;
		uniform float _ShadowSpecularOcclusionPower;
		...
		void surf( Input i , inout SurfaceOutputStandardSpecularCustom o )
		{
			...
			float simplePerlin3D341 = snoise( ( ase_worldPos / _IceNoiseTiling ) );
			float4 lerpResult249 = lerp( ( saturate( 	max( blendOpSrc236, blendOpDest236 ) )) , temp_output_230_0 , simplePerlin3D341);
			...
			float eyeDepth355 = LinearEyeDepth(SAMPLE_DEPTH_TEXTURE_PROJ(_CameraDepthTexture,UNITY_PROJ_COORD( ase_screenPos )));
			float temp_output_94_0 = saturate( pow( ( abs( ( eyeDepth355 - ase_screenPos.w ) ) * _WaterDepth ) , _WaterFalloff ) );
			...
			half4 screenColor305 = tex2D( _GrabScreen1, ( (ase_screenPosNorm).xy + (( tex2DNode17 * _ShadowRefractionPower )).xy ) );
			...
			float4 lerpResult294 = lerp( lerpResult93 , unity_AmbientSky , clampResult299);
			o.Albedo = lerpResult294.rgb;
			float clampResult382 = clamp( ( _ShadowSpecularOcclusionPower * clampResult299 ) , 0.0 , 1.0 );
			float lerpResult371 = lerp( _SpecularPower , _ShadowSpecularPower , clampResult382);
			#ifdef _SHADOWSPECULAROCCLUSION_ON
				float staticSwitch379 = lerpResult371;
			#else
				float staticSwitch379 = _SpecularPower;
			#endif
			float3 temp_cast_3 = (staticSwitch379).xxx;
			o.Specular = temp_cast_3;
			o.Smoothness = _Smoothness;
			o.Translucency = lerpResult93.rgb;
